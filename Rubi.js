var OZ = {
    ...
    DOM: {
        ...
        append: function() {
            if (arguments.length == 1) {
                ...
                for (var i = 0; i < arguments.length; i++) { OZ.DOM.append(arguments[i]); }
        }
    },
    Style: {
    ...
    Request: function(url, callback, options) {
        ...
        var xhr = false;
        if (window.XMLHttpRequest) { xhr = new XMLHttpRequest(); } else if (window.ActiveXObject) { xhr = new ActiveXObject("Microsoft.XMLHTTP"); } else { return false; }
...
Face.ROTATION = [
    [Face.TOP, Face.FRONT, Face.BOTTOM, Face.BACK].reverse(), [Face.LEFT, Face.BACK, Face.RIGHT, Face.FRONT].reverse(), [Face.LEFT, Face.BOTTOM, Face.RIGHT, Face.TOP].reverse()
];
